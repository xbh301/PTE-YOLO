...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward       用来测试得到stride    float32
            check_anchor_order(m)  # must be in pixel-space (not grid-space)    # 检查anchor顺序和stride顺序是否一致
            m.anchors /= m.stride.view(-1, 1, 1)        # 求出相对当前feature map的anchor大小 如[10, 13]/8 -> [1.25, 1.625]
