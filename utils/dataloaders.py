...
class LoadImages:
    ...
    def __next__(self):
        ...
        img_patches = [None] * 5
        for i in range(5):
            img_patches[i] = transforms.RandomResizedCrop(size=self.img_size[0] // 4, scale=(0.06, 1.0), ratio=(1, 1))(
                img_pil)
            img_patches[i] = np.array(img_patches[i]).transpose((2, 0, 1))
            img_patches[i] = np.ascontiguousarray(img_patches[i])
            # img_patches[i] = torch.from_numpy(img_patches[i])
...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if nl:
            labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=img.shape[1], h=img.shape[0], clip=True, eps=1E-3)        # xyxy to xywh normalized
        ...
        img_patches = [None] * 5
        for i in range(5):
            img_patches[i] = transforms.RandomResizedCrop(size=self.img_size // 4, scale=(0.06, 1.0), ratio=(1, 1))(
                img_pil)
            img_patches[i] = np.array(img_patches[i]).transpose((2, 0, 1))
            img_patches[i] = np.ascontiguousarray(img_patches[i])
            img_patches[i] = torch.from_numpy(img_patches[i])
        ...
        return torch.from_numpy(img), labels_out, self.im_files[index], shapes, img_patches
    ...
    def collate_fn(batch):
        im, label, path, shapes, img_patches = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        img = []
        img.append(torch.stack(im, 0))
        img.append(torch.stack(img_patches, 0))
        return img, torch.cat(label, 0), path, shapes
